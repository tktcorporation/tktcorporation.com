 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */eI("firebase","9.20.0","app");let eB="@firebase/installations",eM="0.6.4",e$=`w:${eM}`,eN="FIS_v2",ej=new B("installations","Installations",{"missing-app-config-values":'Missing App configuration value: "{$valueName}"',"not-registered":"Firebase Installation is not registered.","installation-not-found":"Firebase Installation not found.","request-failed":'{$requestName} request failed with error "{$serverCode} {$serverStatus}: {$serverMessage}"',"app-offline":"Could not process request. Application offline.","delete-pending-registration":"Can't delete installation while there is a pending registration request."});function eP(e){return e instanceof L&&e.code.includes("request-failed")}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function e9({appConfig:e,heartbeatServiceProvider:t},n){let r=function(e,{fid:t}){return`${eF(e)}/${t}/authTokens:generate`}(e,n),a=function(e,{refreshToken:t}){let n=eH(e);return n.append("Authorization",`${eN} ${t}`),n}(e,n),i=t.getImmediate({optional:!0});if(i){let e=await i.getHeartbeatsHeader();e&&a.append("x-firebase-client",e)}let s={installation:{sdkVersion:e$,appId:e.appId}},o={method:"POST",headers:a,body:JSON.stringify(s)},l=await ez(()=>fetch(r,o));if(l.ok){let e=await l.json(),t=ex(e);return t}throw await eR("Generate Auth Token",l)}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
